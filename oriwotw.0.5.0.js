class Route {
    ...
    addBranch(branch) {
        if (branch && this.branches[branch] == undefined) {
            this.branches.push({ index: branch, branch: allMousePosRecords[branch].pathText });
            if (this === activeRoute) {
    ...
    getBranches() {
        var branches = [];
        for (var i = 0; i < this.branches.length; i++) {
            branches.push(allMousePosRecords[this.branches[i].index]);
        }
        return branches;
    }
    ...
    updateBranchesHtml() {
        ...
        for (var i = 0; i < this.branches.length; i++) {
            ...
            allMousePosRecords[this.branches[i].index].frameIndex = frameIndex;
            frameIndex += allMousePosRecords[this.branches[i].index].paths[0].nodes.length;

            clone.querySelector('label.map-elements-current-route-branches-name').textContent = (i + 1) + ' - ' + (allMousePosRecords[key].paths[0].text !== undefined ? allMousePosRecords[key].paths[0].text : '');
            clone.querySelector('label.map-elements-current-route-branches-location').textContent = map.getMapLocation({ x: allMousePosRecords[key].paths[0].nodes[0].x, y: allMousePosRecords[key].paths[0].nodes[0].y });
            clone.querySelector('span.map-elements-current-route-branches-color').style.backgroundColor = allMousePosRecords[key].paths[0].color !== undefined ? allMousePosRecords[key].paths[0].color : 'red';
            clone.children[0].dataset.pos = allMousePosRecords[key].paths[0].nodes[0].x + ' ' + allMousePosRecords[key].paths[0].nodes[0].y;
            clone.children[0].dataset.indexes = allMousePosRecords[key].mapId;
            ...
            switch (event.type) {
                case 'click':
                    if (element.classList.contains('map-elements-route')) {
                        activeRoute = this;
                        activeBranch = getBranch(this.branches[0]);
                        drawer.activeFrame = 0;
                        this.showData();
...
class Branch {
    ...
    validatePathNodeIndex() {
        for (var i = 0; i < this.paths.length; i++) {
            for (var i2 = 0; i2 < this.paths[i].nodes.length; i2++) {
                this.paths[i].nodes[i2].branchIndex = this.mapId;
            }
        }
        this.hasChanged = true;
    }
    ...
    drawPaths() {
        ...
        if (activeBranch !== this) {
            ...
            timeline.setFrameMoverFrame(this.frameIndex);
        }
        if (drawAllFullColor) {
            ctxPaint.globalAlpha = 1.0;
        }
    ...
    duplicateBranch() {
        ...
        data.mapId = allMousePosRecords.length;
        delete data.id;
        data.standard = false;

        var newBranch = new Branch(allMousePosRecords.length);
        newBranch.loadBranch(data);
        newBranch.validatePathNodeIndex();
        allMousePosRecords.push(newBranch);
...
class Path {
    ...
    newPath(path, pathIndex = null) {
        ...
        allMousePosRecords[this.branchIndex].hasChanged = true;
    }
    ...
    createTextNode(node, nodeIndex, nodeTextCont) {
        ...
        label.addEventListener('click', function () { map.setMap({ x: pos.x * -1, y: pos.y * -1 }) });
    ...
    createNodeLinks(node, nodeIndex, nodeTextCont) {
        for (var iL = 0; iL < node.links.length; iL++) {
            ...
            if (linkedElement) {
                ...
                label.addEventListener('click', function () { const pos = this.dataset.mappos.split(' '); map.setMap({ x: pos[0], y: pos[1] }) });
    ...
    handleEvent(e) {
        activeBranch = allMousePosRecords[this.branchIndex];
        allMousePosRecords[this.branchIndex].hasChanged = true;
        switch (e.type) {
            case 'input':
            ...
            case 'change':
                if (e.target.classList.contains('branch-path-child-standard')) {
                    ...
                    allMousePosRecords[this.branchIndex].upload = e.target.checked;
...
class PathNode {
    ...
    setPosition(newPosition) {
        ...
        if (allMousePosRecords[this.branchIndex]) {
            allMousePosRecords[this.branchIndex].hasChanged = true;
        }
    }
...
class Inventory {
    ...
    createHTML() {
        ...
        for (var i = 0; i < this.abilities.length; i++) {
            ...
            if (drawer.activeFrame < ability.frame) {
        ...
        for (var i = 0; i < this.shards.length; i++) {
            ...
            if (addedShards[shard.data.name] === undefined) {
                ...
                if (drawer.activeFrame >= shard.frame) {
        ...
        for (var i = 0; i < this.projectBuilders.length; i++) {
            ...
            if (drawer.activeFrame >= projectBuilder.frame) {
                labelIcon.style.filter = 'drop-shadow(rgb(0, 195, 255) 0px 0px 2px) brightness(100%)';
            } else {
                labelIcon.style.filter = 'brightness(50%)';
            }
        ...
        for (var i = 0; i < this.projectGardeners.length; i++) {
            ...
            if (drawer.activeFrame >= projectGardener.frame) {
                labelIcon.style.filter = 'drop-shadow(rgb(0, 195, 255) 0px 0px 2px) brightness(100%)';
            } else {
                labelIcon.style.filter = 'brightness(50%)';
            }
    ...
    updateAbilities(item) {
        if (item.data.cost) {
            ...
            if (drawer.activeFrame >= item.frame) {
...
class Drawer {
    ...
    clearMap() {
        ...
        for (var i = 0; i < allMousePosRecords.length; i++) {
            for (var i2 = 0; i2 < allMousePosRecords[i].paths.length; i2++) {
                allMousePosRecords[i].paths[i2].removeAnchors();
                allMousePosRecords[i].paths[i2].removeMapNodes();
            }
        }

        if (document.getElementById('strat-vis-toggle').dataset.toggle === 'false') {
    ...
    getToDrawBranches(frame, framesToDraw) {
        var allBranches = allMousePosRecords,
...
class Map {
    constructor() {
        this.currentPosition = { x: -1393.4, y: -227.027 };
    ...
    moveMap(event) {
        if (!isPaintMode) {
            var mapEl = document.getElementById('orimap-container'),
                mapContainer = document.getElementById('map-container'),
                oldTransform = this.currentPosition,
                oriMapSize = document.getElementById('orimap').getBoundingClientRect(),
                oriMapContRect = mapContainer.getBoundingClientRect(),
            ...
            newX = Math.max(newX, (oriMapSize.width * -1 + parseFloat(mapContainer.style.width)));
            newY = Math.max(newY, (oriMapSize.height * -1 + parseFloat(mapContainer.style.height)));
            this.currentPosition = { x: newX, y: newY };
            mapEl.style.transformOrigin = newX + 'px ' + newY + 'px';
            mapEl.style.transform = 'translate(' + newX + 'px, ' + newY + 'px)';
        }
    }
    ...
    setMap(pos) {
        ...
        this.currentPosition = { x: (newX + (oriMapContRect.width / 2)), y: (newY + (oriMapContRect.height / 2)) };
        mapEl.style.transformOrigin = newX + 'px ' + newY + 'px';
        mapEl.style.transform = 'translate(' + (newX + (oriMapContRect.width / 2)) + 'px, ' + (newY + (oriMapContRect.height / 2)) + 'px)';
    }

    zoomMap(event) {
        var mapEl = document.getElementById('mapzoom'),
            mapContainer = document.getElementById('map-container'),
            mapContainer = mapContainer.getBoundingClientRect(),
            fixedDeltaY = event.deltaMode === 1 ? event.deltaY : Math.round(event.deltaY / 33),
            zoomValue = (fixedDeltaY / -50) * this.currentZoom;

        this.currentZoom += zoomValue;
        //mapEl.style.transformOrigin = (currentMapPosition[0] * currentMapZoom) + (mapContainer.width * -1 / 2) + 'px ' + currentMapPosition[1] * currentMapZoom + 'px';
        //mapEl.style.transform = 'translate(' + currentMapPosition[0] * currentMapZoom + 'px, ' + currentMapPosition[1] * currentMapZoom + 'px) scale(' + currentMapZoom + ')';
        mapEl.style.transform = 'scale(' + this.currentZoom + ')';
        mapEl.style.transformOrigin = '50% ' + mapContainer.height / 2 + 'px';
    }
    ...
    showMapLocationNodes() {
        ...
        for (var i = 0; i < keys.length; i++) {
            for (var i2 = 0; i2 < this.allMaps[keys[i]].length; i2++) {
                ...
                button.addEventListener('mouseup', function () { map.showMapLocationNodes(); });
...
class Timeline {
    ...
    setFrameMoverPos(event) {
        if (this.frameDragging || event.type === 'contextmenu') {
            ...
            if (drawer.activeFrame !== frame) {
                drawer.activeFrame = Math.min(frame, parseFloat(container.style.minWidth) / 3);
                drawer.drawFrames(drawer.activeFrame);
            }
        }
    }
}
...
cvsPaint.addEventListener('mouseup', function () {
    ...
    if (activeBranch === null) {
        newBranch = new Branch(allMousePosRecords.length);
    } else {
...
cvsPaint.addEventListener('mousemove', function (event) {
    if (isPainting) {
        ...
        const mPos = { x: (event.clientX - oriMapCont.left) / map.currentZoom, y: (event.clientY - oriMapCont.top) / map.currentZoom };
...
function createNewBranch() {
    var path = new Branch(allMousePosRecords.length);
    activeBranch = path;
    allMousePosRecords.push(path);
...
function createIconsIconPicker() {
    ...
    for (var i = 0; i < keysCategories.length; i++) {
        ...
        label.addEventListener('click', function () { toggleIconsOnMap(this.dataset.category, this); });
        label.textContent = iconDataNewNames[keysCategories[i]];

        div.dataset.category = keysCategories[i];
        div.addEventListener('click', function () { toggleIconsOnMap(this.dataset.category, event.target.dataset.iconName); });
        div.addEventListener('contextmenu', function () { event.preventDefault(); event.stopPropagation(); toggleIconsOnMap(this.dataset.category, event.target.dataset.iconName); var x = document.querySelectorAll('span[data-name="' + event.target.dataset.name + '"]')[0].dataset.pos.split(' '); map.setMap({ x: x[0] * -1, y: x[1] * -1 }); });
        ...
        for (var i2 = 0; i2 < keysIcons.length; i2++) {
            ...
            var info = '';
            switch (iconDataNew[keysCategories[i]][keysIcons[i2]].category) {
                case 'projectGardener':
                    if (iconDataNew[keysCategories[i]][keysIcons[i2]].description) {
                        info += iconDataNew[keysCategories[i]][keysIcons[i2]].description;
                    }
                    if (iconDataNew[keysCategories[i]][keysIcons[i2]].cost) {
                        info += iconDataNew[keysCategories[i]][keysIcons[i2]].cost;
                    }
                    if (iconDataNew[keysCategories[i]][keysIcons[i2]].effect) {
                        info += iconDataNew[keysCategories[i]][keysIcons[i2]].effect;
                    }
                    break;

                case 'shard':
                    if (iconDataNew[keysCategories[i]][keysIcons[i2]].data) {
                        info += iconDataNew[keysCategories[i]][keysIcons[i2]].data;
                    }
                    if (iconDataNew[keysCategories[i]][keysIcons[i2]].upgrades) {
                        info += 'Easy: ' + iconDataNew[keysCategories[i]][keysIcons[i2]].upgrades[0].cost.easy + ' Normal: ' + iconDataNew[keysCategories[i]][keysIcons[i2]].upgrades[0].cost.normal + ' Hard: ' + iconDataNew[keysCategories[i]][keysIcons[i2]].upgrades[0].cost.hard;
                    }
                    break;
            }
...
function clearMouseVars() {
    timeline.frameDragging = false;
    anchorBeingDragged = undefined;
    anchorDragging = false;
    activeBranchIndex = null;
    commentBeingDragged = undefined;
    map.isDraggingMap = false;
    iconBeingDragged = undefined;
    if (activeMapBorderNode !== false) {
        activeMapBorderNode = false;
        map.showMapLocationNodes();
    }
    activeMapBorderNode = false;
}
...
function saveAllBranches() {
    var json = [];
    for (var i = 0; i < allMousePosRecords.length; i++) {
        if (allMousePosRecords[i].standard === false && allMousePosRecords[i].upload !== true || allMousePosRecords[i].standard === true && allMousePosRecords[i].hasChanged && allMousePosRecords[i].upload === false) {
            json.push(allMousePosRecords[i].saveBranch());
        }
    }
    if (json.length > 0) {
    ...
    for (var i = 0; i < allRoutes.length; i++) {
        if (allRoutes[i].standard === false && allRoutes[i].upload !== true || allRoutes[i].standard === true && allRoutes[i].hasChanged && allRoutes[i].upload === false) {
            if (allRoutes[i].hasChanged !== undefined) {
                delete allRoutes[i].hasChanged;
                delete allRoutes[i].upload;
            }
            if (allRoutes[i].id) {
                delete allRoutes[i].id;
            }
            allRoutes[i].standard = false;
            jsonRoutes.push(allRoutes[i]);
        }
    }
...
function getBranch(name) {
    for (var i = 0; i < allMousePosRecords.length; i++) {
        if (allMousePosRecords[i].pathText === name) {
            return allMousePosRecords[i];
        }
    }
    return false;
}
...
function createComment(event) {
    ...
    pos.x = (event.clientX - oriMapCont.left) / map.currentZoom;
    pos.y = (event.clientY - oriMapCont.top) / map.currentZoom;
...
function playFrames() {
    var wasDrawn = drawer.drawFrames(drawer.activeFrame),
    ...
    if (playButton.dataset.active === 'true') {
        drawer.activeFrame++;
        frameMover.style.left = Math.min(drawer.activeFrame * 3, parseFloat(container.style.minWidth)) + 'px';
...
function mouseEvents(event) {
    ...
    var oriMapCont = document.getElementById('orimap-container').getBoundingClientRect(),
        oriMapSize = document.getElementById('orimap').getBoundingClientRect(),
        left = (event.clientX - oriMapCont.left) / map.currentZoom + 'px',
        top = (event.clientY - oriMapCont.top) / map.currentZoom + 'px',
        gameMapTop = ((4796.8700000000003 - (parseFloat(top) * -1)) * -1) * 0.72, //4860.870000000000
        gameMapLeft = (2440.73 - (parseFloat(left).mapRange(0, Math.abs(oriMapSize.width) / map.currentZoom, Math.abs(oriMapSize.width) / map.currentZoom, 0) * 0.726));

    document.getElementById('map-element-mouse-position').textContent = (+parseFloat(left).toFixed(2) * -1) + ', ' + (+parseFloat(top).toFixed(2) * -1);
    document.getElementById('map-element-mouse-position-game').value = +gameMapLeft.toFixed(2) + ', ' + +gameMapTop.toFixed(2);
    document.getElementById('current-map-location').textContent = map.getMapLocation({ x: parseFloat(left), y: parseFloat(top) });

    if (timeline.frameDragging) {
        timeline.setFrameMoverPos(event);
        ...
        map.setMapBorderNodePos(parseFloat(left), parseFloat(top));
    } else if (isInGameBoxMode && isDrawingBox) {
        createBox(event);
    } else if (map.isDraggingMap && isPaintMode === false && isSubPathMode === false && isNodeMode === false) {
        map.moveMap(event);
    }
}

function convertMapY(y) {
    return ((4796.8700000000003 - (parseFloat(y) * -1)) * -1) * 0.72
}

function convertMapX(x, rect) {
    return (2440.73 - (parseFloat(x).mapRange(0, Math.abs(rect.width) / map.currentZoom, Math.abs(rect.width) / map.currentZoom, 0) * 0.726));
}
...
function mouseClickEvents(event) {
    ...
    if (contextMenu != undefined && contextMenu.style.display == 'flex') {
    ...
    if (isNodeMode && activeBranch) {
        ...
        if (event.ctrlKey && event.target.classList.contains('frame-position-button')) {
            ...
            allMousePosRecords[indexes[2]].paths[indexes[1]].removeNode(event.target);
        } else {
        ...
        if (isDrawingBox) {
            ...
            hitboxBoxSize = {
                x: convertMapX((sizePosBox.x > startPosBox.x ? startPosBox.x : sizePosBox.x), oriMapSize),
                y: convertMapY((sizePosBox.y > startPosBox.y ? startPosBox.y : sizePosBox.y)),
                w: convertMapX((sizePosBox.x < startPosBox.x ? startPosBox.x : sizePosBox.x), oriMapSize),
                h: convertMapY((sizePosBox.y < startPosBox.y ? startPosBox.y : sizePosBox.y))
            },
                copyText = '';
                //gameMapTop = ((4796.8700000000003 - (parseFloat(hitboxBoxSize.y) * -1)) * -1) * 0.72, //4860.870000000000
                //gameMapLeft = (2440.73 - (parseFloat(hitboxBoxSize.x).mapRange(0, Math.abs(oriMapSize.width) / map.currentZoom, Math.abs(oriMapSize.width) / map.currentZoom, 0) * 0.726));

            //copyText = gameMapLeft.toFixed(2) + ', ' + gameMapTop.toFixed(2) + ', ';
            copyToClipboard(hitboxBoxSize.x.toFixed(2) + ', ' + hitboxBoxSize.y.toFixed(2) + ', ' + (Math.abs(hitboxBoxSize.w - hitboxBoxSize.x)).toFixed(2) + ', ' + Math.abs((hitboxBoxSize.h - hitboxBoxSize.y)).toFixed(2));
            isDrawingBox = false;
            startPosBox = { x: 0, y: 0 };
        } else {
    ...
    } else if (document.getElementById('context-menu').style.display === 'flex') {
...
function rightMouseClickEvents(event) {
    ...
    if (event.button === 2) {
        ...
        } else if (event.target.classList.contains('branch-path-map-label-text') && allMousePosRecords[event.target.dataset.indexes.split(' ')[0]].standard) {
...
function createBox(event) {
    if (isDrawingBox) {
        ...
        sizePosBox = { x: (event.clientX - oriMapCont.left) / map.currentZoom, y: (event.clientY - oriMapCont.top) / map.currentZoom };
...
function setAnchorPos(event) {
    if (anchorDragging && anchorBeingDragged !== undefined) {
        ...
        pos = allMousePosRecords[indexes[2]].paths[indexes[1]].nodes[indexes[0]];
        pos.x = (event.clientX - sizePerc.left) / map.currentZoom;
        pos.y = (event.clientY - sizePerc.top) / map.currentZoom;

        anchorBeingDragged.style.left = pos.x + 'px';
        anchorBeingDragged.style.top = pos.y + 'px';

        allMousePosRecords[indexes[2]].paths[indexes[1]].nodes[indexes[0]] = pos;
        allMousePosRecords[indexes[2]].hasChanged = true;
        drawer.drawFrames(drawer.activeFrame);
    }
}
...
function setCommentPos(event) {
    if (isEditingMode) {
        ...
        container.style.left = (event.clientX - oriMapCont.left) / map.currentZoom + 'px';
        container.style.top = (event.clientY - oriMapCont.top) / map.currentZoom + 'px';
...
function setIconPos(event) {
    if (isEditingMode) {
        ...
        iconBeingDragged.style.left = (event.clientX - oriMapCont.left) / map.currentZoom + 'px';
        iconBeingDragged.style.top = (event.clientY - oriMapCont.top) / map.currentZoom + 'px';
...
function checkParams(urlParams = undefined) {
    ...
    if (urlParams.has('icon')) {
        ...
        for (var i = 0; i < icons.length; i++) {
            ...
            if (i === 0) {
                map.setMap({ x: parseFloat(icon.x) * -1, y: parseFloat(icon.y) * -1 });
                map.setMapZoomLevel(3.5);
            }
        }
    ...
    if (urlParams.has('path')) {
        activeBranch = allMousePosRecords[urlParams.get('path')];
        allMousePosRecords[urlParams.get('path')].drawPaths();
        map.setMap({ x: allMousePosRecords[urlParams.get('path')].paths[0].nodes[0].x * -1, y: allMousePosRecords[urlParams.get('path')].paths[0].nodes[0].y * -1 });
    }
}
